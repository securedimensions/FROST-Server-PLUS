...
public abstract class TableHelper {
    ...
    protected void assertEmptyDatastream(JooqPersistenceManager pm, Entity datastream) {
        ...
        if (datastream.getPrimaryKeyValues().get(0) != null) {
            ...
            Query query = QueryParser.parseQuery("", pm.getCoreSettings().getQueryDefaults(), pm.getCoreSettings(), rp);
            query.validate();
    ...
    protected void assertEmptyCampaign(JooqPersistenceManager pm, Entity project) {
        ...
        if (project.getPrimaryKeyValues().get(0) != null) {
            ...
            Query query = QueryParser.parseQuery("$expand=Datastreams($top=0;$count=true),MultiDatastreams($top=0;$count=true)", pm.getCoreSettings().getQueryDefaults(), pm.getCoreSettings(), rp);
            query.validate();
